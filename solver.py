...
# Calculates the decision tree. The argument root_id_suffix is necessary to distinguish different nodes
...
def decision_tree_calculation(subset: pd.DataFrame, root_id_suffix: str, detailed_log: bool) -> (str, list[str], list[str]):
    ...
    max_str = "max{" + igs_comma_separated[:-2] + "} = Gain(S," + str(split_attr_name) + ") --> split at " + str(split_attr_name)
    if detailed_log:
        log.append("\t" + max_str)
    ...
    for val in vals:  # Iterate over all values of the split attribute.
        ...
        if amount_of_different_target_attr_vals == 1:  # stops the recursion when there is only one target attribute value left (i. e. when we have perfect entropy)
            ...
            return_val = decision_tree_calculation(val_subset, root_id_suffix + str(id_suffix), detailed_log)  # recursively calculate the decision tree with the split attribute as root node
...
# Creates the decision tree returning the dot file and creates depending on the input argument
# either a compact or a detailed log file.
def decision_tree_creation(input_path: str, detailed_log: bool, output_dir: str):
    # data management
    df: pd.DataFrame = read_csv_file(input_path)
    input_file_name = os.path.basename(input_path)

    # calculation
    output = decision_tree_calculation(df, "", detailed_log)
...
def process_data(input_path: str, detailed_log: bool, output_dir: str, svg: bool, graph_preview: bool, dot: bool, sub_folder: bool) -> None:
    ...
    decision_tree_creation(input_path, detailed_log, output_dir)
